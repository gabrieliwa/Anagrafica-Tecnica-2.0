...
extension AddAssetWizardView {
        ...
        HStack {
            Button {
                ...
                Image(systemName: "xmark")
                    .font(.system(size: AppMetrics.wizardCloseIconSize, weight: .bold))
                    .padding(8)
            ...
            Spacer()
            Color.clear.frame(
                width: AppMetrics.wizardHeaderPlaceholderSize,
                height: AppMetrics.wizardHeaderPlaceholderSize
            )
        }
    }
    ...
    var chooseFamilyView: some View {
            ...
            Button {
                ...
                HStack {
                    ...
                    Image(systemName: "chevron.right")
                        .font(.system(size: AppMetrics.wizardChevronSize, weight: .bold))
                        .foregroundStyle(AppColors.textSecondary)
                }
        ...
        VStack(alignment: .leading, spacing: AppSpacing.md) {
            ...
            TextField("Type name", text: $viewModel.newTypeName)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: AppRadius.field)
                        .stroke(
                            viewModel.showValidationErrors && viewModel.newTypeName.isEmpty ? Color.red : AppColors.cardBorder,
                            lineWidth: AppMetrics.cardStrokeWidth
                        )
                )
        }
    }

    var instanceFormView: some View {
        VStack(alignment: .leading, spacing: AppSpacing.md) {
            Text("Instance form")
                .font(AppTypography.section)
                .foregroundStyle(AppColors.textPrimary)
    ...
    var roomNoteView: some View {
        VStack(alignment: .leading, spacing: AppSpacing.md) {
            ...
            TextField("Description", text: $viewModel.roomNoteDescription, axis: .vertical)
                .textFieldStyle(.plain)
                .padding(AppSpacing.md)
                .frame(minHeight: AppMetrics.wizardTextAreaMinHeight, alignment: .topLeading)
                .background(RoundedRectangle(cornerRadius: AppRadius.field).fill(AppColors.cardBackground))
                .overlay(
                    RoundedRectangle(cornerRadius: AppRadius.field)
                        .stroke(
                            viewModel.showValidationErrors && viewModel.roomNoteBlocked && viewModel.roomNoteDescription.isEmpty
                                ? Color.red : AppColors.cardBorder,
                            lineWidth: AppMetrics.cardStrokeWidth
                        )
                )
        }
        ...
        HStack(spacing: AppSpacing.sm) {
            ForEach(0..<count, id: \.self) { index in
                Circle()
                    ...
                    .frame(width: AppMetrics.wizardStepDotSize, height: AppMetrics.wizardStepDotSize)
                if index < count - 1 {
                    Capsule()
                        .fill(index < currentIndex ? AppColors.accent : AppColors.cardBorder)
                        .frame(width: AppMetrics.wizardStepLineWidth, height: AppMetrics.wizardStepLineHeight)
                }
            }
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: AppSpacing.sm) {
            ...
            Button {
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: AppRadius.field)
                        .stroke(isMissing ? Color.red : AppColors.cardBorder, lineWidth: AppMetrics.cardStrokeWidth)
                )
            }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            case .enumerated:
                ...
                let options = (definition.enumValues ?? []).filter { value in
                    let trimmed = value.trimmingCharacters(in: .whitespacesAndNewlines)
                    return !trimmed.isEmpty && trimmed.lowercased() != "select"
                }
                Menu {
        ...
        .background(
            RoundedRectangle(cornerRadius: AppRadius.field)
                .stroke(showValidation && requiresValue ? Color.red : AppColors.cardBorder, lineWidth: AppMetrics.cardStrokeWidth)
        )
    }
