...
Anagrafica Tecnica is a field survey application designed for conducting technical asset inventories in buildings. The system enables operators to catalog all technical equipment—such as lighting fixtures, HVAC components, network infrastructure, and electrical systems—while working on-site, even without network connectivity.

The solution addresses a critical gap in facility management: the need for accurate, structured, and verifiable asset data collected directly in the field. By combining an offline-first mobile application with a web-based administration dashboard, the system ensures data quality at the point of capture while maintaining the flexibility required for real-world building survey conditions.
...
| Problem | Impact |
|---------|--------|
| **Inconsistent naming** | Different operators use different names for identical equipment, making aggregation and analysis difficult |
| **Missing information** | Without enforced validation, critical fields are frequently left empty |
| **Location ambiguity** | Text-based descriptions like "third floor hallway" are imprecise and error-prone |
| **Duplicate entries** | Same asset types created multiple times with slight variations |
...
Anagrafica Tecnica reimagines the asset survey workflow by placing the building floorplan at the center of the experience. Instead of working from room lists or text-based navigation, operators interact directly with an interactive map of the building, seeing exactly where they are and what has been surveyed.

The system enforces data quality through structured input forms with validation rules, while remaining flexible enough to handle the unpredictable nature of field work. By operating fully offline and synchronizing when connectivity is available, the tool works reliably in any building environment.
...
**Key Features:**
- Interactive floorplan with Google Maps-style pan, zoom, and navigation
...
- Full offline operation with background synchronization
...
| **Pain points** | Connectivity issues, data re-entry, forgetting rooms, photo organization |
...
│ • Required fixes  │  │  • Or upload new schema definition   │
│                   │  │  • Schema locked to project          │
│ → Fix DWG and     │  └──────────────┬───────────────────────┘
│   re-upload       │                 │
└───────────────────┘                 ▼
                       ┌──────────────────────────────────────┐
...
- Enters project name, client information, and description
...
│  • Tap room to select it             │
│  • View room status indicator        │
└──────────────┬───────────────────────┘
               │
               ▼
┌──────────────────────────────────────┐
...
- Selects a project and initiates download
...
- Level picker allows switching between floors
- Rooms are color-coded by status: not started, in progress, completed
- Operator physically walks to the room in the building

**Step 3: Add Asset**
- Operator taps the location on the floorplan where the asset is physically located
- Asset creation wizard opens with Family selection (e.g., Lights, Radiators, Access Points)
- Type selection screen offers three options:
  1. Select existing Type from searchable list
  2. Duplicate existing Type and modify parameters
  3. Create new Type from scratch
- Fuzzy matching suggests similar existing Types to prevent duplicates
- Instance parameter form displays fields defined by the Family schema
...
- Asset is saved locally with marker placed on floorplan

**Step 4: Complete Survey**
- Operator taps "Complete Survey" when finished
...
- Project state changes to COMPLETED; becomes read-only on device
...
│  Recently Used:                      │
│  ┌────────────────────────────────┐  │
│  │  Philips 30W LED Panel         │  │
│  │  Osram 18W Tube Light          │  │
│  └────────────────────────────────┘  │
│                                      │
│  All Types (47):                     │
│  ┌────────────────────────────────┐  │
